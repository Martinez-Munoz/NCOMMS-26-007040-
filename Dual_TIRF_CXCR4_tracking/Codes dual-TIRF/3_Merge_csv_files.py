...
folder_path = os.path.normpath(r"/Volumes/Laura 2023/06102023 DUAL TIRF Rab11")
...
def get_trajectory_densities(data):
    ...
    def extract_group(filename):
        match = re.search(r'(CTL|KO)', filename)
    ...
    for filename in data['filename'].unique():
        ...
        for sampletype, colocalization in product(['big', 'small'], ['positive', 'negative']):
            expected_combinations.append({
                'filename': filename,
                'sampletype': f"{group} {sampletype}",
                'rab_colocalization': colocalization
            })
    
    expected_combinations_df = pd.DataFrame(expected_combinations)
    
    grouped = data.groupby(['filename', 'sampletype', 'rab_colocalization']).size().reset_index(name='count')
    
    # Merge the grouped data with the expected combinations
    result = expected_combinations_df.merge(grouped, on=['filename', 'sampletype', 'rab_colocalization'], how='left')
