...
experiment_to_analyze = "Experiment_3"

# Do you want to plot Rab positive or negative? (it won't affect the output csv)
plot_rab_positive = True
...
def calculate_diffusion_parameters(trajectories):
    ...
    tracks['rab_colocalization'] = trajectories.groupby('TRACK_ID')['Rab colocalization'] \
    .apply(lambda x: x[x.notna()].value_counts().idxmax() if not x[x.notna()].empty else np.nan) \
    .reset_index(drop=True)
...
for i in range(len(video_folder_paths)):
    ...
    with warnings.catch_warnings():
        ...
        try:
            if os.path.exists(save_path + '_Cellpose_cell_mask.tif') and os.path.exists(save_path + '_Tracks_CXCR4.csv') and os.path.exists(save_path + '_Tracks_Rab.csv'):
...
if plot_rab_positive:
    all_outputs = all_outputs[all_outputs["rab_colocalization"] == "positive"]
else:
    all_outputs = all_outputs[all_outputs["rab_colocalization"] == "negative"]
