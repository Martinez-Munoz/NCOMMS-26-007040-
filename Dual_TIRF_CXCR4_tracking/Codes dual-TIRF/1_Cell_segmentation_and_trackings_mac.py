...
folder_name = os.path.normpath(r"/Volumes/Laura 2023/06102023 DUAL TIRF Rab11")
...
experiment_to_analyze = "Experiment_3"  # Set to "experiment1" or None for all
condition_to_analyze = "KO"  # Set to "KO" or None for all
time_point_to_analyze = "45min" # Set to "10 min" or None for all
...
tif_export = True #Do you want to export a tif file of the original video + overlayed tracks? (True=yes, False=no)
...
def ensure_rings(trajectories, min_length, threshold):
    for particle in trajectories.particle.unique():
        ...
        if len(ring_traj) < min_length:
            ...
            for frame in ring_traj.frame:
                ...
                inset_frame = video_cxcr4_norm[frame, xr-r:xr+r, yr-r:yr+r]
                inset_inner_frame = video_cxcr4_norm[frame, xr-2:xr+2, yr-2:yr+2]
                #plt.figure()
...
for i in range(len(video_folder_paths)):
    ...
    try:
        ...
        with warnings.catch_warnings():
            ...
            video = import_video(filepath)
        
        #Splitting the video into the two channels
        video_cxcr4 = video[:,:,0:int(video.shape[2]/2)]
        #video_cxcr4 = video_cxcr4[0:100]
        video_rab = video[:,:,int(video.shape[2]/2):]
        #video_rab = video_rab[0:100]
        
        #Segmenting cells using Cellpose on the time-averaged Rab channel
        image_cells = adjust_gamma(np.mean(video_rab, axis=0), 1) #a gamma filter is applied to help segment cells of varying expression levels
        # plt.figure()
        # plt.imshow(image_cells)
        segmented_cells = cellpose_segmentation(image_cells)
        ...
        trajectories_rings_cxcr4 = ensure_rings(trajectories_rings_cxcr4, min_length=3, threshold=80)
        trajectories_cxcr4 = ring_corrections(trajectories_blobs_cxcr4, trajectories_rings_cxcr4, addRings=False)
        
        video_rab_norm = video_rab/np.max(video_rab, axis=(1,2), keepdims=True)
        localizations_blobs_rab = detect_blobs(video_rab_norm, min_sigma=2, max_sigma=8, threshold=0.03500)
        trajectories_blobs_rab = linking(localizations_blobs_rab, search_range=[3,5,5], allow_n_gaps=1)
        localizations_rings_rab = detect_rings(video_rab_norm, dp=1.4, minDist=8, param1=1, param2=0.6, minRadius=2, maxRadius=15)
        trajectories_rings_rab = linking(localizations_rings_rab, search_range=[3,5,5], allow_n_gaps=3)
        trajectories_rings_rab = ensure_rings(trajectories_rings_rab, min_length=3, threshold=80)
        trajectories_rab = ring_corrections(trajectories_blobs_rab, trajectories_rings_rab, addRings=True)
        ...
        if csv_export:
            ...
            csv_ending = '_Tracks_Rab.csv'
            csv_track_path = save_path + csv_ending
            # The for loop below converts the sigma of blob_log to the radius by multiplying it by a factor of 2^(1/2).
            for arr in localizations_blobs_rab:
                arr[:, 2] *= np.sqrt(2)
            localizations_all_rab = [np.vstack((a, b)) for a, b in zip(localizations_blobs_rab, localizations_rings_rab)]
            export_tracks(trajectories_rab, localizations_all_rab, csv_track_path)
