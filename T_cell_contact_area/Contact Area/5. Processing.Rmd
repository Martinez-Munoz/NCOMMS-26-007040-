---
title: |
  | [MLII] Ejercicios de Evaluación 
  | RNAS y RF
  | Alejandro Campoy López  
  | alecampoy@protonmail.com 
  | dni: 48806295L
output:
...
### Paquetes que vamos a usar en alguno de los ejercicios:
...
# 1. Carga del otro dataset

Las fotos anotadas como borroso aparecen como NA
```{r}
# No disponemos del conteo manual de las celulas por imagen
# df_n_Cell_manual_counted <- read_csv('Datos/n_Cell_auto_completo.csv', col_types = c('ffffiiiii')) %>% pivot_longer(cols = starts_with('t'), names_to = 'Time', values_to = "n_Cell_manual_counted") %>% drop_na()
```

# 2. Agrupamiento de la información de las celulas por imagen para normalizar
...
# 3. Union de ambos DF (no aplica ya que no tengo el archivo de conteo manual de celulas)

Genero una variable que contenga todas las categorias para unir por ahi

```{r}
# df_imagenes <- df_imagenes %>% mutate(gruping_aux = paste(Fecha, Muestra, Condicion, View, Time, sep = "_"))
# No disponemos del conteo manual de las celulas por imagen
# df_n_Cell_manual_counted <- df_n_Cell_manual_counted %>% mutate(gruping_aux = paste(Fecha, Muestra, Condicion, View, Time, sep = "_"))
```

Realizo la union como *inner_join* para quedarme solo con las que tengan información

```{r}
# No disponemos del conteo manual de las celulas por imagen
# df_imagenes <- inner_join(df_imagenes, df_n_Cell_manual_counted %>% select(n_Cell_manual_counted, gruping_aux), by='gruping_aux')
```

Ahora puedo generar el area normalizada, aunque no creo que sea necesario

```{r}
# No disponemos del conteo manual de las celulas por imagen
# df_imagenes$Area_norm <- df_imagenes$Area_suma / df_imagenes$n_Cell_manual_counted
df_imagenes$Area_norm_noOutLiers <- df_imagenes$Area_suma / df_imagenes$n_Cell_no_outliers
```


# 4. Gráfico 

- Primero genero la variable a representar y los factores ordenados

Hay que cambiar los datos a los tiempos correctos que me enviaron por correo
```{r}
...
Odenación del dataset por condición y luego tiempo
